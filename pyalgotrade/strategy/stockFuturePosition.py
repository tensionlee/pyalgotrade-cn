...
from pyalgotrade.strategy.position import PositionState
...
class WaitingEntryState(position.WaitingEntryState):
    ...
    def exit(self, position, stopPrice=None, limitPrice=None, goodTillCanceled=None):
        ...
        position.getStrategy().getSuitableBroker(instrument).cancelOrder(position.getEntryOrder())


class OpenState(position.OpenState):
...
class ClosedState(position.ClosedState):
    # def onEnter(self, position):
...
class stockFuturePosition(Position):
    def __init__(self, strategy, entryOrder, goodTillCanceled, allOrNone):
        # super(stockFuturePosition, self).__init__(strategy, entryOrder, goodTillCanceled, allOrNone)
    ...
    def __submitAndRegisterOrder(self, order):
        ...
        self.getStrategy().getSuitableBroker(order.getInstrument()).submitOrder(order)
...
class LongPosition(stockFuturePosition):
...
class ShortPosition(stockFuturePosition):
    def __init__(self, strategy, instrument, stopPrice, limitPrice, quantity, goodTillCanceled, allOrNone):
        suitableBroker = strategy.getSuitableBroker(instrument)
        if limitPrice is None and stopPrice is None:
